...
function validateScriptLength(script) {
    ...
    if (words < 50) {
        ...
        return { valid: false, error: "Generated script is too short (less than 50 words).", words, estimatedSeconds };

    }

    if (words > 400) {

        return { valid: false, error: "Generated script is too long (over 400 words). May exceed D-ID limits.", words, estimatedSeconds };

    }

    if (estimatedSeconds < 20 || estimatedSeconds > 180) {

        return { valid: false, error: `Script duration (~${Math.round(estimatedSeconds)}s) is outside 20-180s range.`, words, estimatedSeconds };
...
const JOBS_FILE = path.join(DATA_DIR, "video_jobs.json");
const JOB_TTL_MS = 30 * 60 * 1000; // 30 minutes

// Ensure data directory exists (use /tmp on serverless by default)
...
function validateScriptLength(script) {
    ...
    // Detect Arabic: check if >30% of characters are in Arabic Unicode range
    const arabicChars = (script.match(/[\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF]/g) || []).length;
    const isArabic = arabicChars / chars > 0.3;

    // Arabic is spoken slower (~125 wpm) vs English (~150 wpm)
    const wpm = isArabic ? 125 : 150;
    const estimatedSeconds = (words / wpm) * 60;

    if (words < 50) {
        return { valid: false, error: `Script too short (${words} words). Minimum 50 words.`, words, chars, estimatedSeconds };
    }
    if (words > 400) {
        return { valid: false, error: `Script too long (${words} words). Maximum 400 words.`, words, chars, estimatedSeconds };
    }
    if (chars > 5000) {
        return { valid: false, error: `Script too long (${chars} characters). Maximum 5000 characters for D-ID.`, words, chars, estimatedSeconds };
    }
    if (estimatedSeconds < 20) {
        return { valid: false, error: `Script too short (~${Math.round(estimatedSeconds)}s). Minimum 20 seconds.`, words, chars, estimatedSeconds };
    }
    if (estimatedSeconds > 180) {
        return { valid: false, error: `Script too long (~${Math.round(estimatedSeconds)}s). Maximum 180 seconds.`, words, chars, estimatedSeconds };
    }

    return { valid: true, words, chars, estimatedSeconds, language: isArabic ? 'ar' : 'en' };
}
...
async function processVideoJob(jobId, pdfBuffer, options) {
    ...
    try {
        ...
        const basePrompt = normalizedCategory === "explanation"
            ...
            ? "You are an educational presenter. Given the PDF lecture text below, write a clear spoken explanation (90-140 seconds when read aloud, approximately 225-350 words). Explain step-by-step like a tutor, keeping a friendly tone. Match the PDF language (Arabic or English). End with one reflective question. Use plain text sentences, no markdown."

            : "You are an educational presenter. Given the PDF lecture text below, write a concise spoken summary (60-90 seconds when read aloud, approximately 150-225 words). Highlight key points and outcomes. Match the PDF language (Arabic or English). End with one reflective question. Use plain text sentences, no markdown.";

        // Smart PDF slicing to avoid mid-sentence cutoff

        let pdfSlice = pdfText.slice(0, 12000);

        const lastSentenceEnd = Math.max(

            pdfSlice.lastIndexOf('. '),

            pdfSlice.lastIndexOf('.\n'),

            pdfSlice.lastIndexOf('؟ '),  // Arabic question mark

            pdfSlice.lastIndexOf('。'),   // Just in case

        );

        if (lastSentenceEnd > 8000) {

            pdfSlice = pdfSlice.slice(0, lastSentenceEnd + 1);

        }

        const promptSuffix = pdfText.length > 12000

            ? "\n\nNote: This is a partial excerpt from a longer document. Summarize only what is provided below — do not infer or add content beyond what is given.\n\n"

            : "\n\n";

        const combinedText = `${basePrompt}${promptSuffix}PDF content:\n${pdfSlice}`;
        ...
        const assistantText = (scriptText || "").trim();

        // Check for empty or insufficient LLM output

        if (!assistantText || assistantText.split(/\s+/).length < 20) {

            throw new Error("Script generation failed — LLM returned empty or insufficient content. Please try again.");

        }

        // Strip markdown artifacts that LLMs sometimes insert despite instructions

        let cleanedScript = assistantText

            .replace(/^#{1,6}\s+/gm, '')        // Remove heading markers

            .replace(/^\*{1,3}\s?/gm, '')       // Remove bold/italic markers at line start

            .replace(/^-\s+/gm, '')             // Remove list dashes

            .replace(/^\d+\.\s+/gm, '')         // Remove numbered lists

            .replace(/\*{1,3}([^*]+)\*{1,3}/g, '$1')  // Remove inline bold/italic
        ...
        const validation = validateScriptLength(cleanedScript);

        if (!validation.valid) {

            throw new Error(`${validation.error} (${validation.words} words, ~${Math.round(validation.estimatedSeconds)}s)`);

        }



        job.script = cleanedScript;

        job.script_stats = { words: validation.words, estimated_duration: Math.round(validation.estimatedSeconds) };
            ...
            if (apiType === "expressives") {
                ...
                const expressiveResp = await fetch(`${DID_BASE_URL}/expressives`, {
                    ...
                    body: JSON.stringify({
                        ...
                        script: { type: "text", input: cleanedScript },
            ...
            geminiSession = await ai.live.connect({
                ...
                callbacks: {
                    ...
                    onclose: (e) => {
                        ...
                        browserWs.send(JSON.stringify({ type: "session_end", reason: e.reason }));
